...
struct BitmapRef {
    ...
    bool get(int i, int j) {
        size_t pos = (size_t)i * cols + j;
        size_t byte_idx = pos / 8;
        uint8_t test = data[byte_idx] & ((uint8_t)1 << (pos % 8));
        return test != 0;
    }
...
bool is_square_one(uint8_t* data, int num_bytes, int i, int j) {
    const uint8_t* row_i = data + i * num_bytes;
    const uint8_t* row_j = data + j * num_bytes;
    int total_count = 0;

    if constexpr (USE_AVX2) {
        constexpr int SIMD_BYTES = 32;
        const int num_blocks = num_bytes / SIMD_BYTES;

        for (int blk = 0; blk < num_blocks; ++blk) {
            const int offset = blk * SIMD_BYTES;
            __m256i a = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(row_i + offset));
            __m256i b = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(row_j + offset));

            // if a & b == 0
            if (_mm256_testz_si256(a, b)) continue;

            // else
            __m256i x = _mm256_and_si256(a, b);
            uint64_t words[4];
            _mm256_storeu_si256(reinterpret_cast<__m256i*>(words), x);

            for (int k = 0; k < 4; ++k) {
                if (words[k]) {
                    uint64_t cnt = _mm_popcnt_u64(words[k]);
                    total_count += cnt;
        ...
        for (int byte_idx = 0; byte_idx < num_bytes; ++byte_idx) {
            uint8_t byte = row_i[byte_idx] & row_j[byte_idx];
            total_count += nibble_lookup[byte];
            if (total_count >= 2) return false;
        }
    }
...
void BoundaryMask(void* data_ptr, void* dst_ptr, int num_rods) 
{
    auto data = static_cast<uint8_t*>(data_ptr);
    auto dst = static_cast<bool*>(dst_ptr);
    int num_bytes = num_rods / 8;
    BitmapRef mat = BitmapRef(data, num_rods);
    for (int i = 0; i < num_rods; i++) {
        for (int j = i + 1; j < num_rods; j++) {
            if (dst[i] && dst[j])continue;
            if (!mat.get(i, j))continue;
            if (is_square_one(data, num_bytes, i, j)) {
                dst[i] = dst[j] = true;
            }
        }
    }
